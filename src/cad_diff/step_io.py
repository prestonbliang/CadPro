...
def _expand(
    ...
    if not shape_tool.IsAssembly_s(label):
        ...
        solids = _solids_in(located)
        if len(solids) == 1:
            return [(name_hint, solids[0])]
        return [(f"{name_hint}_{index}", solid) for index, solid in enumerate(solids, start=1)]
    ...
    for i in range(1, components.Length() + 1):
        ...
        child_name = _label_name(
            referred,
            fallback=_label_name(comp_label, fallback=f"{name_hint}_{i}"),
        )
