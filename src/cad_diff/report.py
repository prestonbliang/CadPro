...
def _print_face_detail(solid_face_diff, console: Console) -> None:
    ...
    if b.reliable:
        cross_check = (
            f"boolean cross-check: +{b.added_volume:.3f} / -{b.removed_volume:.3f} "
            f"{cubic_mm}, tier 5 ground truth"
        )
    else:
        separator = "—" if unicode_output else "--"
        cross_check = (
            f"[bold red]boolean cross-check: unavailable[/bold red] {separator} the boolean op produced invalid "
